...
ops :: [Op]
ops = [Op "+" (tryOp (+)),Op "-" (tryOp (-)),Op "*" (tryOp (*)),Op "/" tryDiv,Op "^" tryExp]
  where
    ...
    tryExp a b
      ...
      | numerator b < 0 = Just ((1 % 1) / pow a (- numerator b))
      | otherwise = Just (pow a (numerator b))
...
search = [(opseq,numseq,applySeq opseq numseq,applyPairs opseq numseq) | opseq <- opseqs, numseq <- numseqs,applySeq opseq numseq == Just (24%1)]
  where
