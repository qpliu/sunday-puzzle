...
type Stats = (Map (Ranger,Bool) Int,Map (Ranger,Ranger) Int,Map Ranger Int)
...
updateStats (locations,partners,moves) ((a1,b1),(c1,d1)) ((a2,b2),(c2,d2)) =
...
conditions maxLocations maxPartners maxMoves (locations,partners,moves) =
  all (<= maxLocations) locations && all (<= maxPartners) partners && all (<= maxMoves) moves

start :: (Schedule,Stats)
start = ([((A,B),(C,D))],(fromList [((r,l),0) | r <- [A .. D], l <- [True,False]],fromList [((r1,r2),0) | r1 <- [A .. C], r2 <- [succ r1 .. D]], fromList [(r,0) | r <- [A .. D]]))
